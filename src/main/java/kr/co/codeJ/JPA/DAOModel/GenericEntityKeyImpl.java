...
/**
 * Implementation of the GenericEntityKey
 * @author julu1 <julu1 @ naver.com >
 * @version 0.1.0
  */
public class GenericEntityKeyImpl implements GenericEntityKey {
	 ...
	 */
	public GenericEntityKeyImpl(Class<?> type) {
		this.type = type;
	}
	/**
	 * get column names 
	...
	public boolean isValid() throws InvalidNullableException, DBTypeException{
		...
		for ( int i=0; i < columns.length; i++) {
			...
			try {
				String Res=String.valueOf(type.getMethod(getMethodName).invoke(this));
				if ( Res == null )
				{
					if ( exception == null)
	...
	public int hashCode() throws DBTypeException{
		...
		for ( int i=0; i < columns.length; i++) {
			...
			String getMethodName="get" + ColumnName.toUpperCase().charAt(0) + ColumnName.substring(1);
			try {
				columnValues[i]=String.valueOf(type.getMethod(getMethodName).invoke(this));
			} catch (Exception e) {
				throw new DBTypeException(type.getSimpleName(), e.getMessage());
			}
		}
